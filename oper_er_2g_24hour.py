...
interval = int(sys.argv[4]) if len(sys.argv) >= 5 else 2
print(type(interval))

to_sql_table_do = 'append'
...
def engine1():
    ...
    tablename = "day_er_2g_" + server + '_' + 'day'.lower()
    storage = 'temp/day_er_2g_' + server + '_' + interval_name + '.csv'
    ...
    df400 = df200.groupby(["LAC", "CELL_NAME", "SITE_NAME", 'Element', 'Server', 'BAND',
                           'PERIOD_DURATION', 'DD']).agg(agglist)
    ...
    df401.to_sql(tablename, engine, if_exists=to_sql_table_do, index=False, dtype=dtyp)
